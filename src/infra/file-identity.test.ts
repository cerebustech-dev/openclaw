...
describe("sameFileIdentity", () => {
  it.each([
    ...
    {
      name: "accepts win32 dev mismatch when both dev values are non-zero (lstat vs fstat volume-id quirk)",
      left: stat(249400, 11),
      right: stat(3535365016, 11),
      platform: "win32" as const,
      expected: true,
    },
    {
      name: "still rejects ino mismatch on win32 even with dev mismatch",
      left: stat(249400, 11),
      right: stat(3535365016, 12),
      platform: "win32" as const,
      expected: false,
    },
    {
