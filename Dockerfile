...
# `error setting certificate file`. `curl` is also missing from slim and
# is required by the bun-install step that follows; on the full image
# both packages are already present so this is a no-op there.
...
# Note: bun's install.sh requires `unzip` (added to apt-install above).
